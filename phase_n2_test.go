...
func TestWriteAction_NextActionGlyph(t *testing.T) {
	...
	out := evo.NewWithOptions(evo.To(&uniBuf), evo.Plain(), evo.NoColor(), evo.Glyphs(evo.GlyphsUnicode))
	out.Item("done").OK().Next(evo.Label("repo-retire --retire demo"))
	if err := out.Finish(); err != nil {
		t.Fatal(err)
	}
	...
	out2 := evo.NewWithOptions(evo.To(&asciiBuf), evo.Plain(), evo.NoColor(), evo.Glyphs(evo.GlyphsASCII))
	out2.Item("done").OK().Next(evo.Label("repo-retire --retire demo"))
...
func TestWriteProblem_EvidenceGlyph_ASCII(t *testing.T) {
	var buf strings.Builder
	out := evo.NewWithOptions(evo.To(&buf), evo.Plain(), evo.NoColor(), evo.Glyphs(evo.GlyphsASCII))
...
func TestConfirm_ASCIIProfile_PromptGlyph(t *testing.T) {
	var buf strings.Builder
	out := evo.NewWithOptions(evo.To(&buf), evo.Stdin(strings.NewReader("y\n")), evo.Glyphs(evo.GlyphsASCII), evo.NoColor())
