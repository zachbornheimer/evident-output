...
// Each iterates items, driving absolute Progress(i, len(items)) and
// Phase(item) before each item is yielded — the bar reads "items completed
// so far", not "items completed including the one still running", so it
// never shows full while the last item is in flight. Evo owns the counter:
// because progress is set from the loop index rather than a hand-maintained
// counter, re-running work for an item inside the loop body cannot
// double-count or move the bar backward — only advancing to the next item
// does. Normal completion of the loop seals progress at total/total.
//
