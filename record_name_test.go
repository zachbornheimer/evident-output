...
func TestRecordName_PlanOmitsQuantityInPlain(t *testing.T) {
	...
	out := evo.Init(evo.Config{Options: []evo.Option{evo.Title("plan"), evo.To(&buf), evo.Plain(), evo.NoColor()}})
...
func TestRecordName_ChangesOmitsQuantityInPlain(t *testing.T) {
	t.Parallel()
	var buf bytes.Buffer
	out := evo.Init(evo.Config{Options: []evo.Option{evo.Title("done"), evo.To(&buf), evo.Plain(), evo.NoColor()}})
