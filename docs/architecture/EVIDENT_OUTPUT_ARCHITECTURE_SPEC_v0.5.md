...
```go
import "github.com/zachbornheimer/evident-output"
...
Agent assistance follows the same boundary. The portable CLI-output skill is the discovery and bootstrap layer; the implemented Go MCP server is a local stdio execution layer launched by a compatible agent host. No `evident-output.dev` domain, hosted MCP service, or GitHub Pages runtime is required. GitHub is the canonical source, release, integrity, documentation, and integration-distribution surface.
...
- whether a remote MCP service is ever justified; version 1 requires no hosted domain or remote service and distributes the implemented Go MCP locally through GitHub-hosted skills, plugins, extensions, release binaries, and pinned `go run` fallbacks;
...
Evo supports two distinct interactive debug presentations over the same structured log records.

**History mode** is the safe default. Records are durable terminal history written above the live region using Evo's compact bracketed grammar:
...
```text
12:04:18.286 [DEBUG] branch comparison completed  blockers=2 duration=11ms
...
The terminal driver clears the transient region, appends queued history records, and redraws the live region.

**Pane mode** reserves a bounded viewport at the bottom of the live region. It is intended for noisy interactive diagnostics. The newest record appears first, and records use conventional `slog` text syntax rather than Evo's bracketed history grammar:
...
── debug · newest first ────────────────────────────────────────
time=2026-07-27T12:04:18.286Z level=DEBUG msg="branch comparison completed" blockers=2
time=2026-07-27T12:04:18.284Z level=DEBUG msg="fetched remote metadata" remote=origin
time=2026-07-27T12:04:18.241Z level=DEBUG msg="enumerated local branches" count=7
```

The pane is a transient view, not the canonical log. Full records remain available to configured sinks, files, journals, and structured projections in chronological order.
...
`SlogHandler` preserves `slog.Record` structure. History mode applies the compact bracketed human formatter; pane mode applies a standard text-handler-style `key=value` formatter with `time`, `level`, and `msg` fields first and RFC 3339 timestamps by default.
...
type LogFormat interface { /* sealed */ }
...
func LogSink(writer io.Writer, format LogFormat) Option
func SlogText() LogFormat
func JSONLines() LogFormat
func Plain() Option
...
Diagnostic records are durable in Evo's internal journal and in every configured durable sink. Their terminal presentation is selected independently:

```go
func DebugHistory() Option
func DebugPane(options ...DebugPaneOption) Option
func PaneHeight(lines int) DebugPaneOption
func NewestFirst() DebugPaneOption
func OldestFirst() DebugPaneOption
```

`DebugHistory` is the default interactive policy. It appends records above the live region using the compact history grammar:

```text
12:04:18.219 [DEBUG] package index loaded  packages=18
```

`DebugPane` creates a bounded TTY-only pane at the bottom of the live region. Its default ordering is newest-first and its default human grammar follows conventional `slog` text output:

```text
time=2026-07-27T12:04:18.219Z level=DEBUG msg="package index loaded" packages=18
```

Pane ordering changes only the viewport. Durable files, journals, JSON Lines, and downstream handlers preserve chronological record order unless a sink explicitly documents otherwise.

When pane mode cannot be used safely—non-TTY output, insufficient height, `TERM=dumb`, projection-only mode, or terminal ownership loss—Evo SHALL fall back to history or static diagnostic output according to the configured fallback policy. It SHALL NOT emit cursor control merely to preserve the pane.

`DebugWriter` buffers partial lines, validates UTF-8, bounds line length, sanitizes control bytes, and never writes directly to a managed terminal.
...
Every debug call produces one structured record in the Evo journal. Terminal placement is a projection policy; it does not change the record's meaning, identity, timestamp, attributes, redaction status, or chronological sequence.

#### 21.3.1 History mode

History mode is the default. It uses append-above-and-redraw:

1. acquire the shared terminal coordinator;
2. erase the current live region;
3. append queued records above the region;
4. render each record using the compact bracketed grammar;
5. redraw the complete live region;
6. release the coordinator.

Example:

```text
12:04:18.203 [DEBUG] opened repository  path=/work/bpp-csharp
12:04:18.219 [DEBUG] branch comparison completed  blockers=2
...
History records are durable terminal scrollback. The level token is uppercase and bracketed. Color, when enabled, applies primarily to the level token. Timestamps and secondary fields are subdued.

#### 21.3.2 Rolling pane mode

Pane mode reserves a bounded debug viewport **below tasks and items and above the terminal cursor**. The pane belongs to the replaceable live region. It is not durable terminal scrollback.

The default pane behavior SHALL be:

- TTY-only;
- fixed or budgeted height;
- newest record first;
- conventional `slog` text syntax;
- `time`, `level`, and `msg` fields first, with time in RFC 3339 form by default;
- remaining attributes rendered as escaped `key=value` pairs;
- one physical row per record when width permits;
- safe truncation or continuation when it does not;
- no ANSI or cursor control in non-interactive fallback;
- removal from ordinary final output unless preservation is explicitly configured.
...
── debug · newest first ────────────────────────────────────────
time=12:04:18.286 level=DEBUG msg="branch comparison completed" blockers=1
time=2026-07-27T12:04:18.284Z level=DEBUG msg="fetched remote metadata" remote=origin
time=2026-07-27T12:04:18.241Z level=DEBUG msg="enumerated local branches" count=7
```

A pane height of `N` exposes at most `N` formatted records after accounting for its heading and terminal budget. New records enter at the top; older visible records move downward and eventually leave the viewport. The internal journal and configured file or structured sinks remain oldest-first.

The pane SHALL NOT starve the primary CLI UI. When terminal height is constrained, Evo reduces pane height before hiding failed items, warnings, active tasks, or the final conclusion. The pane MAY collapse entirely and fall back to history mode or a one-line indicator according to policy.

On normal completion, the default renderer removes the pane and emits only the stable command result. On failure, cancellation, or explicit preservation policy, Evo MAY append a bounded diagnostic tail beneath the final result, clearly labeled and still formatted as `slog` text. Preservation MUST be opt-in or severity-policy-driven and MUST NOT silently replace a configured durable log sink.

#### 21.3.3 Shared stream coordination
...
### 28.3 Transport and deployment boundary

`evident-output-mcp` supports local stdio in version 1. A compatible agent host launches the implemented Go executable as a subprocess and communicates through stdin and stdout.

Under stdio:

- stdin and stdout are reserved exclusively for MCP JSON-RPC messages;
- all server logs use stderr;
- no human banner, spinner, ANSI frame, startup advice, download status, or shell prompt is printed to stdout;
...
- a launcher, wrapper, or bootstrap script MUST preserve the same stdout-purity requirement;
- process exit, signal handling, and stderr diagnostics remain observable to the host.

Version 1 SHALL NOT require:

- an `evident-output.dev` domain;
- a hosted HTTP MCP service;
- GitHub Pages as an executable backend;
- a permanent system-wide installation;
- adding the Evo runtime library to the target application's `go.mod`.

GitHub Pages MAY host static documentation, schemas, examples, and browser-only previews. It cannot be treated as the MCP process. A future remote transport requires a separate security and operations decision and is not part of the version 1 bootstrap contract.
...
### 28.7 Skill-first discovery and local MCP bootstrap

MCP cannot bootstrap its own discovery. An agent cannot call Evident Output tools until an external instruction, plugin, extension, project configuration, or user action has made the server available.

The portable **CLI Output** skill is therefore the primary discovery layer. It SHALL remain useful when:

- Evident Output is not installed in the target project;
- no MCP server is connected;
- the standalone review CLI is unavailable;
- the project deliberately chooses another rendering library.

The skill teaches understandable CLI-output design first and recommends Evident Output only when adoption materially simplifies the implementation.

The canonical skill SHALL use trigger-oriented frontmatter equivalent to:
...
The exact packaging metadata MAY vary by agent host, but the semantic trigger description SHALL remain generated from one canonical source.

#### 28.7.1 Skill behavior

When invoked, the skill SHALL direct the agent to:

1. identify what the command needs to communicate;
2. inspect the existing output implementation and project dependencies;
3. determine whether Evident Output is already present;
4. use Evident Output MCP tools when connected;
5. otherwise use the standalone reviewer when available;
6. otherwise apply the skill's bundled static guidance;
7. avoid adding the dependency merely because the skill is active;
8. recommend adoption only when there is a concrete complexity or correctness benefit;
9. review and preview the completed output;
10. repeat deterministic review until `recheck_required` is false.

Material adoption signals include:

- several independently updating progress rows;
- live output mixed with `slog` or debug records;
- parallel items with grouped evidence;
- narrow-terminal, no-color, ASCII, CI, and non-TTY requirements;
- equivalent human and structured output;
- repeated terminal-region or alignment logic;
- inconsistent output conventions across several commands.

A single durable `fmt.Println` call is not, by itself, sufficient reason to add the library.

#### 28.7.2 Capability fallback order

The skill SHALL use this ordered capability model:

```text
1. Connected Evident Output MCP tools
   → retrieve guidance, review, repair, re-review, and preview

2. Standalone Evident Output CLI available
   → invoke equivalent guide, review, explain, and preview commands

3. Neither tool available
   → apply the bundled skill guidance and examples directly
```

The skill MUST NOT fail merely because MCP is unavailable. Tool availability changes the strength of verification, not whether the skill can help.

#### 28.7.3 Local Go MCP launch strategies
...
1. **Pinned GitHub Release binary — preferred user path**
   ...
   - the binary is cached outside the target project;
...
3. **Pinned `go install` command — developer path**
   ```text
   go install github.com/zachbornheimer/evident-output/cmd/evident-output-mcp@vX.Y.Z
   ```
   - useful for contributors and Go-centric teams;
   - not required for skill-only use;
   - the host configuration invokes the installed executable by path or name.

The integration SHALL produce a clear diagnostic on stderr when no supported launch path is available. It SHALL never silently fall back to an unpinned version.

#### 28.7.4 Host integrations

The repository SHALL ship generated, thin integrations for major coding agents:

```text
/.github/skills/cli-output/
/skills/cli-output/
...
Each integration SHOULD bundle or reference:

- the canonical CLI Output skill;
- a local stdio MCP definition;
- a pinned executable version;
- a launcher or exact `go run` fallback;
- the specialized reviewer subagent where the host supports one;
- setup and removal instructions;
- an explicit statement that the MCP process is local.

Where an agent host supports plugins or extensions containing both skills and MCP configuration, that packaging is preferred over asking users to install two unrelated artifacts. Installing only the raw skill provides guidance but cannot, by itself, guarantee that the host registers an MCP process.

Host-specific descriptors SHALL be generated from a canonical integration manifest. The repository SHALL test them for syntax, pinned-version consistency, command correctness, and drift.

#### 28.7.5 Specialized subagent

The specialized subagent instructions own repetitive guidance retrieval, file review, repair, re-review, and preview generation. The main agent receives:

- a compact implementation summary;
- changed files;
- review findings and rule IDs;
- preview profiles examined;
- unresolved design choices;
- whether MCP, CLI fallback, or static skill guidance was used.

The subagent SHALL not install dependencies, edit MCP configuration, or download executables without the permissions and interaction model required by the host.

#### 28.7.6 Project-local persistence

After a project adopts Evident Output, an optional explicit bootstrap command MAY generate repository-local instructions and MCP configuration:

```text
evident-output init agents
evident-output init skill
evident-output init mcp
```

Such commands SHALL:

- preview intended files before writing unless a noninteractive confirmation flag is supplied;
- preserve existing user-authored instructions;
- pin the integration version;
- avoid embedding absolute developer-machine paths;
- support clean removal;
- never run automatically because the skill was selected.

Generated project guidance SHALL tell future agents to use the existing Evo dependency rather than repeatedly recommending installation.
...
### 28.11 GitHub-native installation and distribution

GitHub is the canonical distribution surface. The project SHALL publish:

- source and tags;
- signed or checksummed MCP and CLI release binaries;
- release manifests by operating system and architecture;
- the portable CLI Output skill;
- host-specific plugin and extension packages;
- schemas, rules, examples, and conformance fixtures;
- optional static documentation through the repository or GitHub Pages.

No hosted Evident Output domain is required.

Recommended acquisition paths:

```text
portable guidance:
  install or copy the repository's cli-output skill

plugin/extension:
  install the host-specific Evident Output integration from GitHub
...
The project MAY provide configuration-generation commands:
...
By default, configuration commands print deterministic configuration. Any command that writes user or repository files SHALL be a separate, explicit operation with preview, conflict handling, and confirmation semantics.

Release and integration manifests SHALL pin exact versions. Generated examples SHALL not use `@latest` in persistent MCP configuration.
...
10. remain useful with only the portable skill installed;
11. launch the local Go MCP through a pinned integration without a hosted domain;
12. fall back to the CLI or bundled guidance when MCP startup is unavailable;
13. avoid recommending or installing the Evo runtime dependency for trivial output.
...
12. portable skill installation and skill-only fallback guide;
13. local stdio MCP launch guide covering release binaries, pinned `go run`, and `go install`;
14. host plugin/extension installation and removal guide;
15. GitHub release integrity, version pinning, and offline-cache guide;
16. guide catalog with task-oriented use cases;
17. migration guide from `fmt.Printf`, ad hoc spinners, and progress libraries;
18. testkit and preview-matrix guide;
19. security model;
20. performance and limits guide;
21. contributing, rule-authoring, integration-generation, and API-review guide;
22. generated reference for every stable review rule and diagnostic.
...
## 64. Counterpoint: “A rolling pane can hide history or make logs appear reversed.”

**Objection:** A newest-first bottom pane is visually useful but violates normal chronological reading order, discards older visible records, and may lead users or agents to mistake the viewport for the complete log.

**Resolution integrated:** Pane mode is explicitly a transient projection over the canonical journal. It uses conventional `slog` text syntax, labels itself `newest first`, has a strict visible-height bound, and never changes durable sink ordering. History mode remains the default. Pane mode is TTY-only, yields screen space before primary command state, disappears on normal completion, and requires an explicit policy to preserve a diagnostic tail. Verification IDs LOG-017 through LOG-025 and red tests H.18 through H.21 enforce this distinction.
...
## 65B. Counterpoint: “The MCP cannot help an agent discover itself.”

### Objection

A local MCP server is useful only after an agent host knows that it exists and has a launch configuration. Depending on MCP as the primary discovery channel creates a circular installation problem. Depending on an Evident Output domain creates unnecessary operational cost and a single external point of failure.

### Integrated resolution

The portable CLI Output skill is the discovery layer and remains useful without MCP or the Evo runtime package. GitHub distributes the canonical skill, host integrations, release binaries, schemas, and instructions. Where supported, plugins and extensions bundle the skill with a pinned local stdio MCP configuration. The launch order is pinned release binary, pinned `go run` fallback, then explicit developer installation. MCP absence degrades to the standalone CLI and then bundled static guidance. No hosted domain is required, the raw skill never silently installs software, and verification IDs MCP-051 through MCP-072 enforce bootstrap behavior.

## 65C. Counterpoint: “The spec is too broad for v1.”
...
- `evident-output-mcp` local stdio server;
- `list_guides`, `get_guidance`, `review`, `preview`, and `explain` tools;
- resources and prompts;
- portable `.github/skills/cli-output` and agent-neutral skill package;
- generated Claude Code, Codex, Gemini, Grok, and OpenCode integrations;
- pinned GitHub Release binaries and checksums;
- pinned `go run` fallback configuration;
- specialized reviewer subagent;
- skill-only, CLI-fallback, and local-MCP end-to-end tests;
- conformance, bootstrap, integrity, and security tests;
- client installation, removal, and project-local bootstrap examples;
- no hosted-domain dependency.
...
```text
12:04:18.219 [DEBUG] batch loaded  documents=200

✓  index  indexed 200 documents
```
...
task.Phase("reading documents")
logger.Debug("batch loaded", "documents", 200)
logger.Debug("document parsed", "name", "README.md")
task.Donef("indexed %d documents", 200)
```

```text
⠋  index  reading documents

── debug · newest first ────────────────────────────────────────
time=2026-07-27T12:04:18.221Z level=DEBUG msg="document parsed" name=README.md
time=2026-07-27T12:04:18.219Z level=DEBUG msg="batch loaded" documents=200
```
...
### H.18 Rolling debug pane is bounded, newest-first, and uses `slog` text syntax

```go
func TestDebugPane_IsNewestFirstBoundedAndSlogFormatted(t *testing.T) {
    clock := testkit.NewClock(time.Date(2026, 7, 27, 12, 4, 18, 0, time.UTC))
    screen := testkit.NewScreen(
        testkit.Interactive(),
        testkit.Width(80),
        testkit.Height(20),
    ...
    out.Debug("enumerated packages", evo.Int("count", 18))
    clock.Advance(time.Millisecond)
    out.Debug("loaded package", evo.String("name", "react"))
    clock.Advance(time.Millisecond)
    out.Debug("loaded package", evo.String("name", "esbuild"))
    clock.Advance(time.Millisecond)
    out.Debug("resolution complete", evo.Int("packages", 18))

    got := screen.LiveText()
    want := `⠋  dependencies  resolving packages

── debug · newest first ────────────────────────────────────────
time=2026-07-27T12:04:18.003Z level=DEBUG msg="resolution complete" packages=18
time=2026-07-27T12:04:18.002Z level=DEBUG msg="loaded package" name=esbuild
time=2026-07-27T12:04:18.001Z level=DEBUG msg="loaded package" name=react
`
    ...
    if strings.Contains(got, "enumerated packages") {
        t.Fatal("pane exceeded configured three-record height")
    }
}
```

### H.19 Newest-first pane does not reverse durable sink order

```go
func TestDebugPane_DurableSinkRemainsChronological(t *testing.T) {
    var logFile bytes.Buffer
    screen := testkit.NewScreen(testkit.Interactive(), testkit.Width(80))

    out := evo.New(
        evo.Terminal(screen),
        evo.DebugPane(evo.PaneHeight(2), evo.NewestFirst()),
        evo.LogSink(&logFile, evo.SlogText()),
    )
    t.Cleanup(func() { _ = out.Close() })

    out.Debug("first", evo.Int("sequence", 1))
    out.Debug("second", evo.Int("sequence", 2))
    out.Debug("third", evo.Int("sequence", 3))

    got := logFile.String()
    first := strings.Index(got, `msg=first`)
    second := strings.Index(got, `msg=second`)
    third := strings.Index(got, `msg=third`)

    if !(0 <= first && first < second && second < third) {
        t.Fatalf("durable sink is not chronological:\n%s", got)
    }
}
```

### H.20 Normal finish removes the transient debug pane

```go
func TestDebugPane_NormalFinishRemovesPane(t *testing.T) {
    screen := testkit.NewScreen(testkit.Interactive(), testkit.Width(80))
    out := evo.New(
        evo.Terminal(screen),
        evo.DebugPane(evo.PaneHeight(3), evo.NewestFirst()),
    )
    t.Cleanup(func() { _ = out.Close() })
    ...
    got := screen.FinalText()
    if strings.Contains(got, "── debug") || strings.Contains(got, "level=DEBUG") {
        t.Fatalf("final output retained transient pane:\n%s", got)
    }

    want := "✓  dependencies  installed 18 packages\n"
    if got != want {
...
### H.21 Pane mode falls back safely outside a TTY

```go
func TestDebugPane_NonInteractiveFallbackHasNoCursorControl(t *testing.T) {
    var diagnostics bytes.Buffer
    out := evo.New(
        evo.Diagnostics(&diagnostics),
        evo.NonInteractive(),
        evo.DebugPane(evo.PaneHeight(3), evo.NewestFirst()),
    )
    t.Cleanup(func() { _ = out.Close() })
    ...
    for _, forbidden := range []string{"\x1b[", "\r", "── debug"} {
        if strings.Contains(got, forbidden) {
            t.Fatalf("non-interactive diagnostics contain %q:\n%s", forbidden, got)
        }
    }

    if !strings.Contains(got, `level=DEBUG`) ||
        !strings.Contains(got, `msg="package index loaded"`) {
        t.Fatalf("static fallback did not preserve slog text record:\n%s", got)
    }
}
```

### H.22 Non-interactive output contains no terminal controls
...
### H.23 Human and JSON projections preserve meaning
...
### H.24 Multiple progress rows preserve declaration order
...
### H.25 Screen budgeting favors failures and active tasks
...
### H.26 High-frequency progress is coalesced but final state is exact
...
### H.27 Skill remains useful without MCP

```go
func TestSkillOnlyModeImprovesCLIOutput(t *testing.T) {
    result := agenttest.Run(t, agenttest.Scenario{
        Skill:      "cli-output",
        MCP:        agenttest.Unavailable,
        ReviewCLI:  agenttest.Unavailable,
        Repository: "testdata/agent/fmt-progress",
        Request:    "Improve this command's CLI output.",
    })

    if result.FailedBecauseToolMissing {
        t.Fatal("skill-only mode treated MCP as mandatory")
    }
    if !result.UsedBundledGuidance {
        t.Fatal("skill-only mode did not use bundled guidance")
    }
    if !result.ReportedVerificationLimit {
        t.Fatal("skill-only mode did not disclose reduced verification")
    }
}
```

### H.28 Pinned `go run` starts MCP without modifying the target module

```go
func TestPinnedGoRunDoesNotModifyTargetModule(t *testing.T) {
    repo := testkit.CopyFixture(t, "testdata/mcp/clean-go-module")
    beforeMod := testkit.ReadFile(t, repo.Join("go.mod"))
    beforeSum := testkit.ReadOptionalFile(t, repo.Join("go.sum"))

    server := mcptest.StartGoRun(t, repo, mcptest.GoRunConfig{
        Module:  "github.com/zachbornheimer/evident-output/cmd/evident-output-mcp",
        Version: "v0.5.0",
    })
    server.Initialize(t)

    if got := testkit.ReadFile(t, repo.Join("go.mod")); got != beforeMod {
        t.Fatal("go run modified target go.mod")
    }
    if got := testkit.ReadOptionalFile(t, repo.Join("go.sum")); got != beforeSum {
        t.Fatal("go run modified target go.sum")
    }
}
```

### H.29 Launcher preserves MCP stdout purity

```go
func TestLauncherWritesOnlyProtocolToStdout(t *testing.T) {
    process := mcptest.StartLauncher(t, mcptest.LauncherConfig{
        Version: "v0.5.0",
        Asset:   mcptest.ValidReleaseAsset(),
    })

    process.Initialize(t)

    for _, frame := range process.StdoutFrames() {
        if err := mcptest.ValidateJSONRPCFrame(frame); err != nil {
            t.Fatalf("non-protocol stdout frame %q: %v", frame, err)
        }
    }
    if strings.Contains(process.StdoutText(), "downloading") {
        t.Fatal("launcher printed acquisition status to MCP stdout")
    }
}
```

### H.30 Release asset integrity failure prevents execution

```go
func TestLauncherRejectsTamperedReleaseAsset(t *testing.T) {
    process := mcptest.StartLauncher(t, mcptest.LauncherConfig{
        Version: "v0.5.0",
        Asset:   mcptest.TamperedReleaseAsset(),
    })

    err := process.Wait()
    if !errors.Is(err, mcptest.ErrIntegrity) {
        t.Fatalf("error = %v, want integrity failure", err)
    }
    if process.ServerStarted() {
        t.Fatal("tampered MCP binary was executed")
    }
    if process.StdoutText() != "" {
        t.Fatalf("integrity diagnostic contaminated stdout: %q", process.StdoutText())
    }
}
```

### H.31 Capability fallback is MCP, then CLI, then static guidance

```go
func TestSkillCapabilityFallbackOrder(t *testing.T) {
    tests := []struct {
        name string
        mcp  agenttest.Availability
        cli  agenttest.Availability
        want agenttest.GuidanceSource
    }{
        {"MCP available", agenttest.Available, agenttest.Available, agenttest.FromMCP},
        {"CLI fallback", agenttest.Unavailable, agenttest.Available, agenttest.FromCLI},
        {"static fallback", agenttest.Unavailable, agenttest.Unavailable, agenttest.FromSkill},
    }
    ...
    for _, test := range tests {
        t.Run(test.name, func(t *testing.T) {
            result := agenttest.Run(t, agenttest.Scenario{
                ...
                MCP:       test.mcp,
                ReviewCLI: test.cli,
                Request:   "Review this CLI progress implementation.",
            })

            if result.GuidanceSource != test.want {
                t.Fatalf("source = %q, want %q", result.GuidanceSource, test.want)
            }
        })
    }
}
...
### H.32 Trivial output does not trigger dependency adoption

```go
func TestSkillDoesNotRecommendEvoForOneDurableLine(t *testing.T) {
    result := agenttest.Run(t, agenttest.Scenario{
        Skill:   "cli-output",
        Source:  `func run() { fmt.Println("configuration is valid") }`,
        Request: "Make this CLI output clear.",
    })

    if result.RecommendedRuntimeDependency {
        t.Fatal("skill recommended Evident Output for trivial durable output")
    }
}
```

### H.33 Bundled integration exposes the skill and local MCP together

```go
func TestGeneratedIntegrationBundlesSkillAndLocalMCP(t *testing.T) {
    integrations := integrationtest.LoadGenerated(t, "integrations")

    for _, integration := range integrations {
        t.Run(integration.Name, func(t *testing.T) {
            if !integration.HasSkill("cli-output") {
                t.Fatal("integration omitted cli-output skill")
            }
            server := integration.MCPServer("evident-output")
            if server == nil {
                t.Fatal("integration omitted local MCP configuration")
            }
            if server.Transport != "stdio" {
                t.Fatalf("transport = %q, want stdio", server.Transport)
            }
            if !server.VersionIsPinned() {
                t.Fatal("MCP version is not pinned")
            }
        })
    }
}
...
### H.34 Shared conformance runner
