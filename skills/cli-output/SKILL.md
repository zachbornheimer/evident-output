...
**Pinned release:** `v0.2.12` (keep install commands on this pin; never `@latest`).
...
| **MCP install (module)** | `GOBIN=$HOME/.local/bin go install github.com/zachbornheimer/evident-output/cmd/evident-output-mcp@v0.2.12` |
...
2. **Standalone CLI** — `go run github.com/zachbornheimer/evident-output/cmd/evident-output@v0.2.12 …`  
...
```bash
go run github.com/zachbornheimer/evident-output/cmd/evident-output@v0.2.12 review ./path.go
```
