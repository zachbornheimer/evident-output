...
**Pinned release:** `v0.3.3` (keep install commands on this pin; never `@latest`).
...
| **MCP install (module)** | `GOBIN=$HOME/.local/bin go install github.com/zachbornheimer/evident-output/cmd/evident-output-mcp@v0.3.3` |
...
2. **Standalone CLI** — `go run github.com/zachbornheimer/evident-output/cmd/evident-output@v0.3.3 …`
...
```bash
go get github.com/zachbornheimer/evident-output@v0.3.3
```
...
```bash
go run github.com/zachbornheimer/evident-output/cmd/evident-output@v0.3.3 review ./path.go
```
