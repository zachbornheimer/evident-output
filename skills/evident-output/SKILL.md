...
**Pinned release:** `v0.2.12` (never install `@latest` for persistent tooling).

| What | Path |
|------|------|
| Module | `github.com/zachbornheimer/evident-output` |
| MCP binary target | `$HOME/.local/bin/evident-output-mcp` |
| MCP install (pinned) | `GOBIN=$HOME/.local/bin go install github.com/zachbornheimer/evident-output/cmd/evident-output-mcp@v0.2.12` |
